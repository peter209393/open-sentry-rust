...
export default function Home() {
  ...
  useEffect(() => {
    if (!user) return;
    api<ProjectListItem[]>("/api/projects").then(items => {
      const active = items.filter(item => !item.archived_at);
      setProjects(items);
      const remembered = window.localStorage.getItem("open-sentry-project-id");
      const next = active.find(item => item.id === remembered)?.id
        ?? active.find(item => item.id === projectId)?.id
        ?? active[0]?.id;
  ...
  function switchProject(id:string){window.localStorage.setItem("open-sentry-project-id",id);setSelected(null);setFixContext(null);setProjectId(id);setMenuOpen(false);}
  async function createProject(e:React.FormEvent){e.preventDefault();const p=await api<ProjectListItem>("/api/projects",{method:"POST",body:JSON.stringify({name:newProjectName,slug:newProjectSlug})});setProjects(v=>[...v,p]);setNewProjectName("");setNewProjectSlug("");switchProject(p.id);}
    ...
    <div className="app-shell">
      <aside className={`sidebar ${menuOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="project-switcher">
          <span className="project-avatar">{project?.name?.[0]?.toUpperCase() ?? "D"}</span>
          <label><span>当前 Project</span><select aria-label="切换 Project ID" value={projectId} onChange={event=>switchProject(event.target.value)}>{projects.filter(item=>!item.archived_at).map(item=><option key={item.id} value={item.id}>#{item.external_id} · {item.name}</option>)}</select><small>{project?.slug ?? "production"} · {projectId.slice(0,8)}</small></label>
        </div>
      </aside>
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          ...
          {view === "projects"&&<div className="management-grid"><section className="panel management-panel"><p className="section-kicker">PROJECTS</p><h2>项目与数据边界</h2><div className="rule-list">{projects.filter(p=>!p.archived_at).map(p=><article key={p.id}><span className="project-avatar">{p.name[0]}</span><span><strong>{p.name}</strong><small>{p.slug} · Project #{p.external_id} · UUID {p.id.slice(0,8)}… · 保留 {p.retention_days??runtimeConfig?.retention_days??30} 天</small></span><button disabled={p.id===projectId} onClick={()=>switchProject(p.id)}>{p.id===projectId?"当前项目":"切换"}</button>{canManage&&p.id!==projectId&&<button onClick={()=>archiveProject(p.id)}>归档</button>}</article>)}</div></section>{canManage&&<section className="panel management-panel"><p className="section-kicker">CREATE PROJECT ID</p><h2>创建新 Project</h2><p className="management-copy">系统将自动分配唯一 UUID 和 Sentry SDK 使用的数字 Project ID，创建后立即切换。</p><form className="settings-form" onSubmit={createProject}><label>项目名称<input required value={newProjectName} onChange={e=>setNewProjectName(e.target.value)} placeholder="Payment API"/></label><label>Slug<input required pattern="[a-z0-9-]+" value={newProjectSlug} onChange={e=>setNewProjectSlug(e.target.value.toLowerCase())} placeholder="payment-api"/></label><button>创建 Project ID</button></form></section>}</div>}
